<?php
	...
	$param_keys=array('ord_id', 'product_id', 'amount', 'price', 'user_id'); // названия полей в таблице БД
	$param_str=array('Заказ', 'Товар', 'Количество', 'Цена', 'Клиент'); // названия столбцов в таблице для отображения
	$param_ext=array(0, '`products`.`name`', 0, 0, '`users`.`fio`'); // поля для select
	$param_need='ord_id'; // обязательные поля, без которых не сохранять данные
...
<?php
	...
	$query="
		SELECT
			...
			CONCAT(`users`.`login`, ' (', `users`.`fio`, ') ', `users`.`phone`) AS 'Пользователь'
			$admin_delete
			$admin_edit
		FROM
			`$table`
