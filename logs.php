<?php
	...
	$param_keys=array('event_type_id', 'dt', 'user_id', 'stat_id', 'note'); // названия полей в таблице БД
	$param_str=array('Действие', 'Дата и время', 'Пользователь', 'Код товара или категории', 'Поисковый запрос'); // названия столбцов в таблице для отображения
	$param_ext=array('`event_types`.`name`', 0, '`users`.`fio`', 0, 0); // поля для select
...
<?php
	...
	$query="
		SELECT
			logs.id AS 'Номер',
			event_types.name AS 'Действие',
			logs.dt AS 'Дата и время',
			users.fio AS 'Пользователь',
			CASE
				WHEN logs.event_type_id = 3 THEN categories.name
				WHEN logs.event_type_id = 4 THEN products.name
				ELSE ''
			END AS 'Наименование категории или товара',
			logs.note AS 'Поисковый запрос'
			$admin_delete
			$admin_edit
		FROM logs
		LEFT JOIN users ON users.id = logs.user_id
		LEFT JOIN event_types ON event_types.id = logs.event_type_id
		LEFT JOIN categories ON categories.id = logs.stat_id AND logs.event_type_id = 3
		LEFT JOIN products ON products.id = logs.stat_id AND logs.event_type_id = 4
		ORDER BY logs.dt DESC, logs.id DESC
	";
