<?php
	...
	$param_keys=array('name', 'value', 'start', 'stop'); // названия полей в таблице БД
	$param_str=array('Наименование акции/скидки', 'Значение, %', 'Дата начала', 'Дата окончания'); // названия столбцов в таблице для отображения
	$param_ext=array(0, 0, 0, 0); // поля для select
	$param_need='name'; // обязательные поля, без которых не сохранять данные
	$dates=array('start', 'stop'); // поля типа "дата"
...
<?php
	...
	if (!empty($_GET['delete_id'])) {
		...
		// каскадное редактирование в таблице товаров
		$query="
			UPDATE `products`
			SET
				discount_id=0
